...
import { apiUnits, apiExams, apiExamQuestions, apiMarkChoice,apiCompletePaper } from "../utils/network";
...
function ExamsScreen({ navigation }) {
  ...
  const getExams = (xtoken, unit) =>{
      let unitid = 'all';
      if(unit){unitid = unit;}
      apiExams(xtoken, unitid)
      .then( (res) => {
  ...
  const getCurrentQuestions = (examid, title, time) =>{
    ...
    .then( (res) => {
      ...
      {
        Alert.alert("Exam Warning", res.message,
        [{text:"Okay"}]);
        return;
      }
    }).catch((error) => {
      console.log(error);
      Alert.alert("Exam Warning", error,
      [{text:"Okay"}]);
      return;
    });
}
...
const initiateExam = (examid, title, duration) => {
  ...
  Alert.alert("START " + title + "?", "We highly recommend that you access this exam on your computer by visiting WEL elearning website. If you are ready to attempt on your phone, Click on Start, Once you start, you will not be allowed to close until you finish. You have " + duration + "min to complete.",
...
const handleExamFilter = async () => {
    ...
    await getExams(token, selectedValue);
...
const finishCurrentExam = (id) => {
  ...
  .then((resp) => {
    ...
    if(resp.status === 200)
    {
      Alert.alert("Exam Notification", "Exam has been submitted for marking, check your performance for more information", [{text:"Okay, Thanks", onPress: () => {
...
const updateChoice = (choice, index, this_exam, question) => {
            ...
            <View style={{flex:1, flexDirection:"row"}}>
                <View style={[globalStyle.text, {flex:1}]}>
                  <TouchableOpacity style={styles.lessonMeta} onPress={() => console.log(undefined) }>
                      <Icon style={styles.lessonIcon} name="ios-pricetag" size={35} />
                      <Text>{mark}mks</Text>
                  </TouchableOpacity>
                </View>
                <View style={[globalStyle.text, {flex:1}]}>
                  <TouchableOpacity style={styles.lessonMeta} onPress={() => console.log(undefined) }>
                      <Icon style={styles.lessonIcon} name="ios-time" size={35} />
  ...
  const ExamItem = ({ id, index, title, this_exam, options, mark}) => (
    <TouchableHighlight style={[globalStyle.listItem, {marginBottom:15, marginTop:10, minHeight:300}]} underlayColor="#f1f1f1">
      <Animatable.View animation="bounceIn" duration={10}>
        <View style={[globalStyle.row, { marginBottom: 4 }]}>
          <View style={[globalStyle.text, {flex:1}]}>
            <Text style={styles.lessonTitle}>
            ...
            <View style={pstyles.cardContainer}>
              <View style={[styles.gridColumn,{flexDirection:"row"}]}>
                  <View style={[styles.gridCell, {flex:6, backgroundColor:colors.container_grey}]}>
                    <Text style={styles.gridCellTitle}>All Exams</Text>
                  </View>
              ...
              <View style={[styles.gridColumn,{flexDirection:"row"}]}>
                  ...
                  <View style={[styles.gridCell, {flex:2, backgroundColor:colors.container_grey}]}>
                    <TouchableOpacity style={styles.signIn}>
                        <LinearGradient colors={[colors.primary_dark, colors.primary]} style={[styles.signIn,{flexDirection:"row"}]}>
                        <CountDown onFinish={() => finishCurrentExam(progress.exam)} digitStyle={{backgroundColor:colors.primary,}} timeLabels={{}} timeToShow={['H', 'M', 'S']} style={[styles.gridCellText, {backgroundColor:colors.white}]} until={progress.duration*60} size={16}/>
                        </LinearGradient>
                    </TouchableOpacity>
                  </View>
                </View>
        ...
        { progress.showInst === false && progress.isOn === false && (
          <ScrollView>
            <View style={pstyles.cardContainer}>
              ...
              {exam.length === 0  && (
                  <View style={{paddingTop:10}}>
                      <Text style={styles.warn}>You have no pending exams. go to performance to see your results</Text>
                  </View>
              )}
              {/* end meta */}
            </View>
